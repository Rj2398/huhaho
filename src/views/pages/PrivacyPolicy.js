...
//   const {getPolicy, getLoading} = useCommon();
...
//   // Translate the "Privacy Policy" title
//   const tPrivacyPolicy = useTranslate('Privacy Policy');

//   useEffect(() => {
//     if (getPolicy?.data && getPolicy.data.length > 0) {
//       const apiData = getPolicy.data[0];
//       const privacyHtml = apiData.privacy_policy || ''; // The HTML content for privacy policy
...
//       const plainText = htmlToText(privacyHtml, {
//         wordwrap: false,
//         selectors: [
...
//       // However, for typical privacy policies, a direct text is usually preferred.
//       // For now, we'll directly set the plain text.
//       setPlainTextContent(plainText);
//     } else {
//       // Reset states if getPolicy data is not available or empty
//       setPlainTextContent('');
//       setImageUrls([]);
//     }
//   }, [getPolicy]); // Dependency array: Effect runs when 'getPolicy' changes

//   // Console logs for debugging (can be removed in production)
//   console.log('Privacy Policy Plain Text Content:', plainTextContent);
//   console.log('Privacy Policy Image URLs:', imageUrls);
...
//       <TopHeader showBack={true} title={tPrivacyPolicy} />
...
const PrivacyPolicy = () => {
  ...
  useEffect(() => {
    ...
    if (getPolicy?.data && getPolicy.data.length > 0) {
      ...
      setPrivacyHtmlContent(apiData.privacy_policy || '');
    } else {
      // If no data, reset the HTML content
      setPrivacyHtmlContent('');
    }
  }, [getPolicy]); // Dependency array: Effect runs when 'getPolicy' object changes

  // Console log for debugging the HTML content being processed
  console.log('Privacy Policy HTML Content for Rendering:', privacyHtmlContent);
        ...
        */}
        {privacyHtmlContent ? (
          <RenderHtml
            ...
            source={{html: privacyHtmlContent}}
